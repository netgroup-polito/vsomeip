...
bench_publish_subscribe_publisher::bench_publish_subscribe_publisher(uint32_t _number_messages, bool _call_service_sync,
...
void bench_publish_subscribe_publisher::run() {
    ...
    for (std::uint32_t payload_size = 1; payload_size <= max_allowed_payload; payload_size *= 2) {
        ...
        call_service_sync_ ? send_messages_sync(its_lock_ack, payload) : send_messages_async(its_lock_ack, payload);
        bench_measurer.stop();
...
void bench_publish_subscribe_publisher::send_messages_sync(std::unique_lock<std::mutex> &_its_lock,
...
void bench_publish_subscribe_publisher::send_messages_async(std::unique_lock<std::mutex> &_its_lock,
                                                            const std::shared_ptr<vsomeip::payload> &_payload) {
    for (number_of_sent_messages_ = 0;
         number_of_sent_messages_ < number_of_messages_to_send_;
         number_of_sent_messages_++, number_of_sent_messages_total_++) {
...
int main(int argc, char **argv) {
    ...
    bench_publish_subscribe_publisher bench_client(number_messages, call_service_sync, shutdown_service_at_end);
